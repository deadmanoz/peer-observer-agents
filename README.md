...
Alertmanager webhook
  ...
  peer-observer-agent (Rust HTTP server)
        ...
        |
        |--> [optional] debug.log (via WireGuard nginx)
        |         |
        |         '-->  Filtered log lines injected into prompt
        |
...
3. If not suppressed, the agent pre-fetches data concurrently — Bitcoin Core RPC snapshots, Parca CPU profiles (for CPU/thread alerts), and debug.log tails (filtered by alert-relevant log categories) — then calls Claude Code CLI with a Prometheus MCP server
